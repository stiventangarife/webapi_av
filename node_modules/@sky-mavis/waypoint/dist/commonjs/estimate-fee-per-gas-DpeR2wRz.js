...
var headless_error_client = require('./headless/error/client.js');
var headless_action_helpers_request_request = require('./headless/action/helpers/request/request.js');
var headless_action_helpers_txTypeCheck = require('./headless/action/helpers/tx-type-check.js');
var saigon = require('./saigon-DYBomzZQ.js');
...
class BaseFeeScalarError extends base.BaseError {
  constructor() {
...
class Eip1559FeesNotSupportedError extends base.BaseError {
  constructor() {
...
class MaxFeePerGasTooLowError extends base.BaseError {
  constructor({
    maxPriorityFeePerGas
  }) {
    super(`\`maxFeePerGas\` cannot be less than the \`maxPriorityFeePerGas\` (${formatGwei.formatGwei(maxPriorityFeePerGas)} gwei).`, {
      name: 'MaxFeePerGasTooLowError'
    });
  }
}

class BlockNotFoundError extends base.BaseError {
  constructor({
    blockHash,
    blockNumber
  }) {
...
function formatTransaction(transaction) {
  const transaction_ = {
    ...
    chainId: transaction.chainId ? toBytes.hexToNumber(transaction.chainId) : undefined,
    ...
    nonce: transaction.nonce ? toBytes.hexToNumber(transaction.nonce) : undefined,
...
async function getBlock(client, {
  ...
  const blockNumberHex = blockNumber !== undefined ? toBytes.numberToHex(blockNumber) : undefined;
  let block = null;
  if (blockHash) {
...
async function internal_estimateMaxPriorityFeePerGas(client, args) {
  ...
  try {
    ...
    return toBytes.hexToBigInt(maxPriorityFeePerGasHex);
  } catch {
...
const GAS_SUGGESTION_ROUTES = {
  [saigon.ronin.id]: "get https://wallet-manager.skymavis.com/proxy/public/v1/ronin/gas-suggestion",
  [saigon.saigon.id]: "get https://wallet-manager-stg.skymavis.one/proxy/public/v1/ronin-testnet/gas-suggestion"
};
...
const fetchEIP1559GasSuggestion = async (client, chainId) => {
  ...
  if (!route) throw new headless_error_client.HeadlessClientError({
    cause: undefined,
  ...
  try {
    const {
      data
    } = await headless_action_helpers_request_request.request(route);
    if (!data) throw new headless_error_client.HeadlessClientError({
      cause: undefined,
      code: headless_error_client.HeadlessClientErrorCode.PrepareTransactionError,
...
const handleEIP1559Transaction = async (client, params) => {
  ...
  return {
    gasPrice: "0x0",
    maxPriorityFeePerGas: maxPriorityFeePerGas || toBytes.numberToHex(gasSuggestion.maxPriorityFeePerGas),
    maxFeePerGas: maxFeePerGas || toBytes.numberToHex(gasSuggestion.maxFeePerGas)
  };
};
const handleLegacyTransaction = async (client, gasPrice) => {
  ...
  return {
    gasPrice: toBytes.numberToHex(bufferedGasPrice),
    maxPriorityFeePerGas: "0x0",
    maxFeePerGas: "0x0"
  };
};
async function estimateFeesPerGas(client, params) {
  ...
  try {
    if (headless_action_helpers_txTypeCheck.isEIP1559CompatibleTransaction(type)) return await handleEIP1559Transaction(client, params);
...
exports.GAS_PRICE_BUFFER_PERCENTAGE = GAS_PRICE_BUFFER_PERCENTAGE;
exports.GAS_SUGGESTION_ROUTES = GAS_SUGGESTION_ROUTES;

...
var utils = require('./utils-U8Z-__N5.js');
...
class Keccak extends utils.Hash {
  ...
  keccak() {
    if (!utils.isLE) utils.byteSwap32(this.state32);
...
function keccak256(value, to_) {
  const to = to_ || 'hex';
  const bytes = keccak_256(size.isHex(value, {
    strict: false
  }) ? toBytes.toBytes(value) : value);
  if (to === 'bytes') return bytes;
  return toBytes.toHex(bytes);
}

...
var headless_action_sendTransaction_serializeTx = require('./serialize-tx.js');
...
const _sendPaidTransaction = async params => {
  ...
  const serializedTx = headless_action_sendTransaction_serializeTx.serializeTX(txInServerFormat);
  const keccakSerializedTx = keccak256.keccak256(serializedTx, "bytes");
  ...
  try {
    ...
    const signResultPromise = headless_action_helpers_triggerSign.wasmTriggerSign(signHandler, keccakSerializedTx, clientShard);
...
const sendPaidTransaction = async params => {
  ...
  const tracker = headless_track_track.createTracker({
    event: headless_action_helpers_txTypeCheck.isEIP1559CompatibleTransaction(transaction.type) ? headless_track_track.HeadlessEventName.endEIP1559Transaction : headless_track_track.HeadlessEventName.sendLegacyTransaction,
    waypointToken,
...
exports.sendPaidTransaction = sendPaidTransaction;
